...
setupBasic2 = function(szprod = 100,lzprod = 100, bent=150, # Productivities
                       temps=10, tempb=8, # Temperature at the top 100 m and bottom
                       nSizeGroups=6, # Number of size groups
  ...
  param$ixR = 1:4 # 4 resources: Small - large zoo small - large benthos 
  param$r = c(1, 1, 1, 0) # Resource growth rates g ww/m2/yr
  param$K = c(szprod, lzprod, param$bprod, 0)  # g ww/m2
  param$mc = c(2e-06*sqrt(500), 0.001*sqrt(500), 0.5e-03*sqrt(250000), 0.25*sqrt(500)) # weight central size)
  param$u0[param$ixR] = param$K # Initial conditions at carrying 
  # Add fish groups:
  #
...
setupVertical = function(szprod= 80,lzprod = 80, # Pelagic productivities
  ...
  param$visual = 1.5 
  param$bent = bent
  bprod=0.1*param$bent #0.1*(param$bent*(param$bottom/param$photic)^-0.86)
  param$bprod = bprod
  param$nSizeGroups = nSizeGroups
  #param$region = region
  param$etaMature=etaMature
  ...
  for (i in param$ixFish) {
    param$sizeprefer[i,] = exp( -(log(param$mc[i]/(beta*param$mc)))^2 / (2*sigma)^2  )
    param$sizeprefer[i,param$mc>param$mc[i]] = 0
  }
  ...
  sigmap = ssigma + tau*log10(param$mc/param$mc[1]) # width for each size class, bigger migrate further
  ...
  for (i in 1: param$nStates) {
    for ( j in 1: param$nStates) {
      test[, j] = pmin(depthDay[, i], depthDay[, j]) #return the minimum biomass
    }
  ...
  TQ10 =  Q10^((tempdata[1:(param$bottom+1)]-10)/10)
  TQ10m =  Q10m^((tempdata[1:(param$bottom+1)]-10)/10)
...
#  A simulation list
# 
simulate= function(p = setupBasic(), tEnd = 100, USEdll=FALSE) {
  #
